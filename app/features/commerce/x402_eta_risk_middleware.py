...
def build_eta_risk_x402_routes(settings: X402Settings) -> RoutesConfig:
    return {
...
def build_eta_risk_payment_middleware(
    ...
    return payment_middleware(
        routes=build_eta_risk_x402_routes(resolved_settings),
        server=server,
        sync_facilitator_on_start=resolved_settings.sync_facilitator_on_start,
    )


def attach_eta_risk_x402_middleware(app: FastAPI) -> None:
    middleware = build_eta_risk_payment_middleware()
    if middleware is None:
        return

    @app.middleware("http")
    async def eta_risk_x402_middleware(
        request: Request,
